...
2. If arguments were provided, interpret them as amendment instructions (new principles, changed constraints, corrected facts). Otherwise, scan the repo for drift: check `AGENTS.md` (the agent source of truth; `CLAUDE.md` symlinks to it), `package.json`, `tsconfig*.json`, and the existing `specs/` files to verify the constitution is accurate.

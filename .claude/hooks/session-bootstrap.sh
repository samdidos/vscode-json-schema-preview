...
# scripts/bootstrap.sh so the actual install logic lives in exactly one place
...
if bash scripts/bootstrap.sh >&2; then
  echo "✓ Environment ready." >&2
else
  echo "✗ Bootstrap failed — run 'bash scripts/bootstrap.sh' manually." >&2
fi
exit 0

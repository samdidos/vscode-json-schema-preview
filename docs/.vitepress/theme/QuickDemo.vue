...
const features = [
  ...
  },
  {
    ...
    gif: 'demo-command-surface.gif',
    desc: 'Every command sits behind one JSON Schema: prefix in the palette and one grouped menu in the editor toolbar — plus a walkthrough that introduces the extension on a fresh install.',
  },
  {
    id: 'inline-binding',
    icon: '📎',
    title: 'Inline Binding',
    gif: 'demo-inline-binding.gif',
    desc: 'Bind a data file by writing $schema into the file itself, so the binding travels with the document instead of living in workspace settings.',
  },
  {
    id: 'schema-catalog',
    icon: '📚',
    title: 'Schema Catalog',
    gif: 'demo-schema-catalog.gif',
  ...
  },
  {
    ...
    icon: '🔧',
    title: 'Refactorings',
    gif: 'demo-refactoring.gif',
    desc: 'Extract a repeated subschema into $defs, inline a $ref, rename a definition everywhere, or drop unused ones — computed as minimal edits, and never changing which documents validate.',
  },
  {
    id: 'schema-diff',
    icon: '🧾',
  ...
  },
  {
    ...
    gif: 'demo-schema-coverage.gif',
    desc: 'Measure a schema against real data: which properties the documents never exercise, and which fields the data carries that the schema never declared.',
  },
]
